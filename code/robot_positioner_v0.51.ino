...
void wait_rotate(int degrees) {
  ...
  float pulses_to_count = abs_degrees*pulses*gear_ratio; //-16608
  ...
  while ((pulse_count < pulses_to_count) && (stop_flag == 0)){
    ...
    enc_return_last = n;
    Serial.print("Encoder Signals, n/last_n: ");
    Serial.print((n));
    Serial.print((enc_return_last));
    Serial.print("\n");

    Serial.print("Current Encoder Count: ");
    Serial.print((pulse_count));
    Serial.print("\n");
    Serial.print("Desired Encoder Count: ");
    Serial.print((pulses_to_count));
    Serial.print("\n");
...
void loop() {
    ...
    if (digitalRead(e_stop) == LOW) { // added this in for the e_stop check
      ...
      if(digitalRead(CCW) == HIGH) {
          ...
          if(currentMillis - previousMilis >= 200 && !digitalRead(home_rot)) {  // short press
            previousMilis = currentMillis;
            Serial.print("home_position: short press\n")
            home_position();
          } else if(currentMillis - previousMilis >= 9500 && digitalRead(home_rot)) { // hold
            previousMilis = currentMillis;
            Serial.print("home_position: long press\n")
            enc_return = 0; // reset encoder timer to 0
